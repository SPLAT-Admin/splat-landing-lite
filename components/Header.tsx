  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-black/90 text-white shadow-md backdrop-blur-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex flex-1 justify-end space-x-8 text-base font-semibold">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
        ...
        <button
          ...
          className="md:hidden p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-crimson-primary rounded-md"
        >
          <svg
            className="w-6 h-6"
            fill="none"
      ...
      <nav
        id="mobile-menu"
        className={`md:hidden px-4 pb-4 space-y-3 text-base font-medium bg-black/95 border-t border-white/10 transition-all duration-300 ease-in-out ${open ? 'block' : 'hidden'}`}
      >
        {navLinks.map((link) => (
          <Link
            key={link.href}
