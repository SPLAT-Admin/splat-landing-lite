...
export default function LandingAdminMerch() {
  ...
  async function updateShopUrl(id: string, shop_url: string) {
    setMessage("Saving shop URL...");
    try {
      const res = await fetch("/api/landingadmin/printify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, shop_url }),
      });
      const data = await res.json();
      if (!res.ok || data.ok === false) {
        throw new Error(data.error || "Failed to update shop URL");
      }
      setMessage(data.message || "Done");
      fetchProducts();
    } catch (err: any) {
      setMessage(err.message || "Unable to update shop URL");
    }
  }
    ...
    <div className="p-8 bg-neutral-950 text-acid-white min-h-screen space-y-6">
      ...
      <section className="p-4 border border-neutral-800 rounded-2xl bg-neutral-900">
        ...
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          <input placeholder="Title" value={newProduct.title} onChange={(e) => setNewProduct({ ...newProduct, title: e.target.value })} className="p-2 bg-neutral-800 rounded-md" />
          <input placeholder="Image URL" value={newProduct.image_url} onChange={(e) => setNewProduct({ ...newProduct, image_url: e.target.value })} className="p-2 bg-neutral-800 rounded-md" />
          <input placeholder="Description" value={newProduct.description} onChange={(e) => setNewProduct({ ...newProduct, description: e.target.value })} className="p-2 bg-neutral-800 rounded-md sm:col-span-2 md:col-span-3" />
          <input placeholder="Blueprint ID" value={newProduct.blueprint_id} onChange={(e) => setNewProduct({ ...newProduct, blueprint_id: e.target.value })} className="p-2 bg-neutral-800 rounded-md" />
          <input placeholder="Print Provider ID" value={newProduct.print_provider_id} onChange={(e) => setNewProduct({ ...newProduct, print_provider_id: e.target.value })} className="p-2 bg-neutral-800 rounded-md" />
          <input placeholder="Price (USD)" value={newProduct.price} onChange={(e) => setNewProduct({ ...newProduct, price: e.target.value })} className="p-2 bg-neutral-800 rounded-md" />
          <select value={newProduct.category_id} onChange={(e) => setNewProduct({ ...newProduct, category_id: e.target.value })} className="p-2 bg-neutral-800 rounded-md">
        ...
        <Button onClick={createProduct} disabled={creating} className="mt-4 bg-acid-white text-black font-semibold">
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((p) => (
              <div key={p.id} className="p-4 border border-neutral-800 rounded-xl bg-neutral-900">
                <img src={p.image_url} alt={p.title} className="rounded-md mb-3 w-full h-48 object-cover" />
                ...
                <input
                  ...
                  defaultValue={p.shop_url || ""}
                  onBlur={(e) => updateShopUrl(p.id, e.target.value)}
                  className="mt-1 p-2 w-full bg-neutral-800 rounded-md text-acid-white"
                />

                <div className="flex gap-2 mt-3">
                  <Button onClick={() => togglePublish(p.id, true)} className="bg-green-600 text-white w-full">
                    Publish
                  </Button>
                  <Button onClick={() => togglePublish(p.id, false)} className="bg-red-600 text-white w-full">
                    Unpublish
                  </Button>
                </div>
              </div>
