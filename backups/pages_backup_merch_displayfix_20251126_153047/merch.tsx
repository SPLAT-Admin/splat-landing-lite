...
export default function MerchPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        console.log("🧩 Fetching merch via local API /api/public-merch-feed");
        const res = await fetch("/api/public-merch-feed");
        const json = await res.json();

        if (!res.ok || !json.ok) throw new Error(json.error || "Failed to load merch feed");
        console.log("✅ Merch feed received:", json.products);
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 py-12">
      ...
      <p className="text-center text-lg text-gray-300 mb-12">
        🔥 Exclusive Drops & Bold Collabs — Limited Runs. Loud Vibes.
      </p>
      ...
      {loading && <p className="text-center text-gray-400 animate-pulse">Loading merch...</p>}
