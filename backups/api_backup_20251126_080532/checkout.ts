...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabase.schema('marketing').from("orders")
      .select(
    ...
    // Upsert customer row
    const { data: customerRow, error: customerError } = await supabase.schema('marketing').from("customers")
    ...
    const { error: updateError } = await supabase.schema('marketing').from("orders")
      .update({
        status: "completed",
