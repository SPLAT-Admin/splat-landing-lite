...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: product, error: productError } = await supabase.schema('marketing').from("products")
    ...
    const { data: order, error: insertError } = await supabase.schema('marketing').from("orders")
      .insert([
        {
