...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { error: insertError } = await supabaseService.schema('marketing').from("ambassador").insert([insertPayload]);
    if (insertError) {
      console.error("Ambassador insert error", insertError);
      return res.status(500).json({ ok: false, error: "Failed to save application." });
    }

    await sendConfirmationEmail("ambassador", insertPayload.email);
