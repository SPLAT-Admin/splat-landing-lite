...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "GET") {
    const { data, error } = await supabaseService.schema('marketing')
      .from("promos")
  ...
  if (req.method === "POST") {
    ...
    if (is_active) {
      await supabaseService.schema('marketing').from("promos").update({ is_active: false }).eq("is_active", true);
    }
  ...
  if (req.method === "PUT") {
    ...
    if (payload.is_active) {
      await supabaseService.schema('marketing')
        .from("promos")
